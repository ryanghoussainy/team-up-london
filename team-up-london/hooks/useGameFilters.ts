...
export default function useGameFilters() {
    ...
    const [tempSkillFilter, setTempSkillFilter] = useState<'all' | 'beginner' | 'intermediate' | 'advanced' | 'expert'>('all');
    ...
    const toggleSportSelection = (sportId: string) => {
        if (tempSelectedSportIds.includes(sportId)) {
            setTempSelectedSportIds(tempSelectedSportIds.filter(id => id !== sportId));
        } else {
    ...
    return {
        ...
        // Temporary states
        tempSkillFilter, setTempSkillFilter,
        tempLocationFilter, setTempLocationFilter,
        tempSelectedDate, setTempSelectedDate,
